...
# 配置の根拠が取れたプラン（波形一致／クロマ一致／音内容アラインの連続ラン）に対して、
# さらに clean vocal 無声マスクと安全境界拡張を掛けるか。
#   False（既定）: 元祖版どおり、確定したプランをそのまま流す。
#                  マスクは連続配置を細切れにし（実測 6区間→12区間）、
#                  末尾ズレの内部伸縮補正まで無効化して品質を落とすため。
#   True         : 別マスター推定の配置にもfail-closedの追加検証を掛ける最厳格動作。
# ※どちらでも「配置の根拠がまったく取れない場合はリップシンク→口元なし安全側」
#   という退避経路は共通で、そこではfail-closed一式がフル装備で働く。
STRICT_MASK_FOR_ESTIMATED_PLACEMENT = False
...
def process_with_youtube(urls, music_path, loop_path, output_path, tmp_dir):
    ...
    # 配置の根拠（波形一致 / クロマ一致 / 音内容アラインのslope≈1連続ラン）が
    # 取れたプランは、元祖版どおり素直にそのまま流す。
    # STRICT_MASK_FOR_ESTIMATED_PLACEMENT=True にすると、別マスター推定
    # （クロマ/音内容アライン）配置に対してだけ、clean vocal無声マスクと
    # 安全境界拡張を掛ける従来のfail-closed動作へ戻せる。
    #   ※既定Falseの理由: このマスクは連続配置を細切れにし（実測 6区間→12区間）、
    #     末尾ズレの内部伸縮補正まで無効化して、元祖版より明確に品質が落ちるため。
    #     根拠がまったく取れない場合は下の else 節でリップシンク→安全側へ退避するので、
    #     「未証明の口を出さない」という設計思想自体は保たれる。
    vocal_silence_ranges = None
    if not STRICT_MASK_FOR_ESTIMATED_PLACEMENT:
        if same_source:
            print("  ✅ 波形で同一音源と確定 → 波形プランだけで完成させます"
                  "（一致区間のMVはこの音そのものの映像）")
        else:
            print("  ✅ メロディ/音内容で配置を確定 → そのプランのまま完成させます")
    else:
    ...
    if _content_align_used:
        ...
        for i, (s, e, mv) in enumerate(seg_plan):
            ...
            if span <= 0.1:
                continue
            r = span / dur
            # ★伸縮の上限は「別テンポ音源かどうか」で変える。
            #   Remixはテンポ自体が違うので、ある程度の伸縮は正当な補正になる。
            #   一方 原曲・Edit系（同一音源のはず）で大きく伸ばす必要が出るのは、
            #   そもそも対応が取れていないサイン。無理に帳尻を合わせると
            #   映像がスローになり口も合わなくなる。
            #   実例: Beat It で41秒長い別バージョンMVを掴んだ際、194秒の区間を
            #         ×1.144（14%）引き伸ばして28秒のズレを吸収し、全編破綻した。
            #   実測では ×1.015〜×1.030 は正常に働いていたので、原曲系は6%を上限とし、
            #   超える区間は補正を諦める（等速のまま＝ズレは継ぎ目に残すが映像は壊れない）。
            _lo, _hi = (0.85, 1.18) if is_rmx_measured else (0.94, 1.06)
            if _lo <= r <= _hi and abs(r - 1.0) > 0.015:   # 中間区間の継ぎ目だけ小補正
                seg_rates[i] = r
                print(f"  🩹 区間{i} 末尾ズレ {span - dur:+.1f}秒 → 内部で×{r:.3f} 伸縮して補正")
            elif abs(r - 1.0) > 0.015 and not is_rmx_measured:
                print(f"  ⏭ 区間{i} 末尾ズレ {span - dur:+.1f}秒（×{r:.3f}）は"
                      f"大きすぎるため補正しません（MVが別バージョンの可能性）")
