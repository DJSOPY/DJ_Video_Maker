...
# 成功区間を後段処理で壊さないためのガード（Cake by the Ocean等のRemix対策）。
#   旧方式/Proで既に高一致で合った区間を、口補正やWhisper再固定が
#   「別の歌詞位置へ飛ぶ大きさ」で動かして壊すのを防ぐ。
#   ① 口の粗補正(apply_mouth_lag)の最大移動量を±0.35秒に制限する。
#      1秒以上動かす必要がある区間は、口補正ではなく配置の再探索が本来必要な
#      ケースなので、ここで大きく動かさない（合っている所を守る方を優先）。
MOUTH_LAG_MAX_SHIFT = 0.35
#   ②' 既にこの相関以上で合っている区間は「成功」とみなしてロックし、口補正で
#       動かさない。ズレ0付近で相関が高い＝もう合っているので触ると壊す。
MOUTH_LAG_LOCK_CORR = 0.45
#   ③ Whisper最終再固定は、文脈一致した単語がこの数に満たなければ採用しない。
#      一致語が少ないと、Whisperの単語時刻は当てにならず既存同期を壊しやすい。
WHISPER_MIN_MATCHED_WORDS = 8
...
def whisper_word_align(anchors, rvoc, ovoc, sr, search=2.5):
    ...
    if len(snapped) < WHISPER_MIN_MATCHED_WORDS:
        print(f"     ℹ️ Forced Alignment: 一致語{len(snapped)}個"
              f"（{WHISPER_MIN_MATCHED_WORDS}未満）→ 従来アンカー維持（既存同期を壊さない）")
        return anchors
    ...
    if len(snapped) < WHISPER_MIN_MATCHED_WORDS:
        print(f"     ℹ️ Forced Alignment: 外れ値除外後の一致語{len(snapped)}個"
              f"（{WHISPER_MIN_MATCHED_WORDS}未満）→ 従来アンカー維持（既存同期を壊さない）")
        return anchors
...
def apply_mouth_lag(anchors, mouth_profile, rvoc, sr, max_lag=4.0, min_shift=0.6,
    ...
    for k in range(len(bounds) - 1):
        ...
        # ★成功区間ロック：ズレ0でも既に相関が十分高い＝この区間は既に合っている。
        #   ここを口補正で動かすと、うまく合っていた口パクを別位置へ飛ばして壊す。
        #   合っている区間は触らない（Cake by the Ocean等で高一致区間を守る）。
        try:
            lag0, corr0, _f0 = _ms.measure_segment_mouth_lag(
                mouth_profile, srt, smv, vt, env, max_lag=0.05, ret_always=True)
        except Exception:
            corr0 = None
        already_locked = (corr0 is not None and corr0 >= MOUTH_LAG_LOCK_CORR)
        if already_locked:
            print(f"     🔒 成功区間ロック：remix{srt[0]:.0f}-{srt[-1]:.0f}s は"
                  f"既に合致（相関{corr0:.2f}）→ 動かさない")
            continue
    ...
    try:
        ...
        try:
            ...
            if mouth_profile is not None:
                ...
                # ★最大移動量を±0.35秒に制限（MOUTH_LAG_MAX_SHIFT）。合っている区間を
                #   別の歌詞位置へ飛ばして壊さないため。大きなズレは配置側の問題であって
                #   口補正で強引に動かすと、うまく合っていた区間まで崩れる。
                anchors, n_mlag = apply_mouth_lag(anchors, mouth_profile, rvoc, SR,
                                                  music_dur=music_dur,
                                                  max_lag=MOUTH_LAG_MAX_SHIFT,
                                                  min_shift=0.15)
                if n_mlag == 0:
        ...
        #
        # ★無条件では上書きしない（項目3）。歌声はWhisperの単語開始が母音側に
        #   寄ったり、伸ばし・リバーブ・反復サビで時刻がズレやすく、映像的には
        #   合っていた口×歌声補正を最終Whisperが巻き戻すことがある。
        #   そこで「Whisper適用前」と「適用後」の同期品質を同じ指標で測り、
        #   本当に良くなった時（またはほぼ同等で単語境界がより正確な時）だけ採用する。
        print("🗣 Forced Alignment最終確定（後段補正後の単語境界を再固定）...")
        anchors_before = [list(a) for a in anchors]
        anchors_whisper = whisper_word_align(anchors, rvoc, ovoc, SR, search=0.55)

        def _sync_quality(anch):
            # 既存の品質レポートから、口×歌声の合い具合を表す代表スコアを取り出す。
            # onset相関（ボーカルと口開きの一致）を主、発音類似を従に評価する。
            try:
                r = alignment_quality_report(
                    anch, windows, rfeat, rt, ofeat, ot, rvoc=rvoc, ovoc=ovoc,
                    sr=SR, rmx_act=rmx_act, feature_kind=fn,
                    mouth_profile=mouth_profile)
                return (float(r.get("onset_correlation", 0.0)) * 1.0
                        + float(r.get("feature_similarity", 0.0)) * 0.5), r
            except Exception:
                return None, None

        _MIN_GAIN = 0.01   # これ以上良くなった時だけWhisper版を採る（僅差は現状維持）
        s_before, _ = _sync_quality(anchors_before)
        s_whisper, _ = _sync_quality(anchors_whisper)
        # 変化量：Whisperが動かした最大秒（100ms以上動く時は特に慎重に）
        max_move = 0.0
        for a, b in zip(anchors_before, anchors_whisper):
            max_move = max(max_move, abs(float(a[1]) - float(b[1])))

        if s_before is None or s_whisper is None:
            # 品質を測れない環境では従来どおりWhisper版（既存挙動を維持）
            anchors = anchors_whisper
            print("     ℹ️ 単語境界を再固定（品質比較は省略）")
        elif s_whisper >= s_before + _MIN_GAIN:
            anchors = anchors_whisper
            print(f"     ✅ Whisper再固定を採用（口×歌声 {s_before:.2f}→{s_whisper:.2f}）")
        elif max_move <= 0.10 and s_whisper >= s_before - _MIN_GAIN:
            # ほぼ同等かつ小さな動きなら、単語境界の正確さを優先してWhisper版
            anchors = anchors_whisper
            print(f"     ✅ Whisper再固定を採用（僅差・単語境界を優先 {s_before:.2f}→{s_whisper:.2f}）")
        else:
            anchors = anchors_before
            print(f"     ↩︎ Whisper再固定は不採用（口×歌声が悪化 {s_before:.2f}→{s_whisper:.2f} / 最大{max_move*1000:.0f}ms動く）→ 補正前を維持")
